...
test -f "$SLIME_REPO/examples/optimizer_geometry/build_sandboxfusion_cgroup2.sh"
test -f "$SLIME_REPO/examples/optimizer_geometry/start_sandboxfusion.sh"
...
export TRAIN_CACHE="$DISK2_ROOT/.training-cache/root-cache"
export SANDBOX_STATE="$HOME/.local/state/slime-opd-geometry/sandboxfusion"
export TRAIN_CONTAINER=iclr2027-mopd-capability
export TRAIN_IMAGE=slimerl/slime:latest

test -f "$SLIME_REPO/examples/mopd_gpas/run_capability_eval.sh"
mkdir -p "$TRAIN_CACHE" "$SANDBOX_STATE"
chmod 700 "$SANDBOX_STATE"
...
进入容器后启动 capability eval（也可以把目标换成单个配置）：
...
当前 capability launcher 对应宿主机 disk2 上的：

```text
/mnt/disk2_from_server2/siqizhu4/iclr2027/slime_opd_geometry/examples/mopd_gpas/run_capability_eval.sh
```
...
只有启动脚本输出全部主动探针通过且 marker 中 `safe` 为 `true`，LiveCodeBench capability eval 才会继续。
